...
// Handle JSON 
DocumentHandler.prototype.handleRawJSON = function(request, response, config) {
  const key = request.params.id.split('.')[0];
  const skipExpire = !!config.documents[key];

  this.store.get(key, function(ret) {
    if (ret) {
      winston.verbose('retrieved json document', { key: key });
      ...
      winston.warn('json document not found', { key: key });
      response.writeHead(404, { 'content-type': 'application/json' });
      if (request.method === 'HEAD') {
        response.end();
      } else {
